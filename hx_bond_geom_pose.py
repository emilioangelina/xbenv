...
for f in lst:
	...
	if (ext == 'pdbqt'): 
		x_atoms = [atom for atom in mol if atom.atype.strip() in HALOGEN if atom.resName.strip() not in ['CL', 'Cl']]
		...
		if car:	
			...
			if OXs:			
				...
				for h in hydrogens:
					...
					for atom in mol:
						if (dist(h,atom) < 1.5) and (atom.element.strip() != 'H'):
							donor = atom 				
							v3 = vector(h, donor)
